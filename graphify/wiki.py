...
def _god_node_article(G: nx.Graph, nid: str, labels: dict[int, str]) -> str:
    ...
    lines: list[str] = []
    lines += [f"# {node_label}", ""]
    lines += [f"> Central node · {G.degree(nid)} connections · `{src}`", ""]
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by graphify. Start here — read community articles for context, then drill into central nodes for detail.",
        "",
    ...
    if god_nodes_data:
        lines += ["## Central Nodes", "(most connected concepts, actors, claims, or events)", ""]
